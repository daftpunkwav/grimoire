...
| DR-002 | The composition root lives only in `apps/api/src/compose.ts`. | Single place to wire port implementations; the rest of the codebase can be reasoned about as data flow through ports. | Code review + `pnpm boundaries` (rejects `compose.ts` from any other file). |
...
| DR-007 | Refresh tokens are stored as sha256 hashes; plaintext is returned exactly once and never persisted. | Allows server-side revocation without keeping the plaintext; protects against DB-only leaks. | `services/identity/src/services/auth.ts` + tests. |
...
| DR-011 | Circuit breaker holds the time source by injection (`Clock`). | Tests can fast-forward without sleeping. | `packages/foundation/src/clock.ts` + per-breaker test. |
